...
# Script tối ưu hóa hình ảnh homestay/hotel sử dụng sips (macOS)
...
# Kiểm tra sips (có sẵn trên macOS)
if ! command -v sips &> /dev/null; then
    echo "❌ sips không khả dụng. Đây là công cụ có sẵn trên macOS."
    exit 1
fi
...
optimize_image() {
    ...
    if [ $size_kb -gt $MAX_SIZE_KB ]; then
        ...
        # Thử giảm chất lượng từ 80% xuống 50%
        for quality in 80 70 60 50; do
            temp_file="${file}.temp"
            cp "$file" "$temp_file"
            
            # Sử dụng sips để giảm chất lượng
            sips -s formatOptions $quality "$temp_file" &>/dev/null
        ...
        for scale in 90 80 70 60; do
            temp_file="${file}.temp"
            cp "$file" "$temp_file"
            
            # Lấy kích thước hiện tại
            width=$(sips -g pixelWidth "$temp_file" | tail -1 | awk '{print $2}')
            height=$(sips -g pixelHeight "$temp_file" | tail -1 | awk '{print $2}')
            
            # Tính kích thước mới
            new_width=$((width * scale / 100))
            new_height=$((height * scale / 100))
            
            # Resize và giảm chất lượng
            sips -z $new_height $new_width "$temp_file" &>/dev/null
            sips -s formatOptions 60 "$temp_file" &>/dev/null
